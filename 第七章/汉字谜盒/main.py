...
#BaseModel 是Pydantic库提供的父类(FastPAI深度继承了Pydantic) ,用于定义FastAPI数据模型和数据验证规则
#与ai交互
@app.post("/api/chat")
def chat(request: ChatRequest):
    ...
    logging.info("<-----ai的回复", {ai_response})
    #5.更新消息列表中的消息
    messages.pop(0)
    messages.append({"role" : "assistant", "content" : ai_response})
    session_data["messages"] = messages
    logging.info("更新后的会话信息", {session_data})
    #6.保存会话数据到json
...
def get_sessions() -> ApiResponse:
    ...
    return ApiResponse(code=200, message="请求成功", data=session_ids)
...
def handle_exception(request: Request, exc: Exception):
    logging.error(f"请求异常:{request.method} {request.url} {exc}")
    return JSONResponse(code=500, message="请求异常", data=None)
